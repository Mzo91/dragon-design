...
function displayAgentName(agent: Pick<AgentInfo, 'id' | 'name'>): string {
  return agent.id === 'amr' ? 'Dragon Design' : agent.name;
}
...
export function AvatarMenu({
  ...
  // whenever the Dragon Design runtime is installed — so the Dragon Design agent row
          ...
          {config.mode === 'daemon' && installedAgents.length > 0 ? (
            <>
              ...
              {installedAgents.map((a) => {
                ...
                // Dragon Design row carries the account (balance + plan) inline,
