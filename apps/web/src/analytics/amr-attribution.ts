...
// Builds the AMR handoff URL with Dragon Design attribution params. When
// `deviceId` is provided it is added as `od_device_id`, so AMR can link the
// landing/registration directly back to this Dragon Design install instead of
// only through the one-shot entry id. The caller passes it ONLY when the user
// has consented to metrics: AMR is Dragon Design's official model service, so
...
async function mirrorAmrEntryToAmrAnalytics(
  ...
  try {
    ...
    // AMR analytics mirroring must never block the primary Dragon Design action.
  }
}
